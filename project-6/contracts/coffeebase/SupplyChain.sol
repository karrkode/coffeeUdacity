...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole {
  ...
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "Item has not for sale.");
    _;
  }
  ...
  function buyItem(uint _upc) public payable
    ...
    checkValue(_upc){
        ...
        address payable payableAddress = address(uint160(items[_upc].consumerID));
        payableAddress.transfer(items[_upc].productPrice);
        emit Sold(_upc);
  }
  ...
  )
  {
    ...
    if (items[_upc].itemState == State.Harvested) {
        itemState = 0;
    } else if (items[_upc].itemState == State.Processed) {
        itemState = 1;
    } else if (items[_upc].itemState == State.Packed) {
        itemState = 2;
        ...
        itemState = 6;
    } else if (items[_upc].itemState == State.Purchased){
        itemState = 7;
    }
